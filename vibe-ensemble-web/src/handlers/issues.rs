...
pub async fn list(State(storage): State<Arc<StorageManager>>) -> Result<Html<String>> {
    ...
    // For now, return the existing HTML implementation
    let mut html = format!(
        r#"
        <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <title>Issues - Vibe Ensemble</title>
            <style>
                ...
                .btn {{ background: #007bff; color: white; padding: 6px 12px; text-decoration: none; border-radius: 4px; font-size: 0.75rem; margin-right: 5px; }}
        ...
        <body>
            <div class="container">
                ...
                <div class="card">
                    <h3>Issues <a href="/issues/new" class="btn btn-primary" style="float: right;">Create New Issue</a></h3>
                    {{TABLE_CONTENT}}
                </div>
            </div>
        </body>
    ...
    if issues.is_empty() {
        html = html.replace("{{TABLE_CONTENT}}", "<p>No issues found. <a href='/issues/new' class='btn btn-primary'>Create First Issue</a></p>");
    } else {
        let mut table_html = String::from("<table><tr><th>Title</th><th>Priority</th><th>Status</th><th>Created</th><th>Actions</th></tr>");
        ...
        for issue in issues {
            table_html.push_str(&format!(
                r#"<tr>
                    ...
                    <td>{}</td>
                    <td><a href="/issues/{}" class="btn">View</a></td>
                </tr>"#,
                issue.title,
                if issue.description.len() > 80 {
                    format!("{}...", &issue.description[..80])
                } else {
        ...
        html = html.replace("{{TABLE_CONTENT}}", &table_html);
    }
