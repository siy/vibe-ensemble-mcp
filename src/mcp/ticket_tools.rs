...
use crate::{
    database::{
        tickets::{Ticket, CreateTicketRequest},
        comments::{Comment, CreateCommentRequest},
    },
    server::AppState,
};
use super::{
    tools::{ToolHandler, extract_param, extract_optional_param, create_success_response, create_error_response},
...
impl ToolHandler for CreateTicketTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        let description: String = extract_optional_param(&Some(args.clone()), "description")?.unwrap_or_default();
        let _ticket_type: String = extract_optional_param(&Some(args.clone()), "ticket_type")?.unwrap_or_else(|| "task".to_string());
        let _priority: String = extract_optional_param(&Some(args.clone()), "priority")?.unwrap_or_else(|| "medium".to_string());

        info!("Creating ticket: {} in project {}", title, project_id);

        let ticket_id = Uuid::new_v4().to_string();
        let execution_plan = vec!["Planning".to_string(), "Implementation".to_string(), "Testing".to_string(), "Review".to_string()];
...
impl ToolHandler for GetTicketTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        match ticket {
            ...
            None => Ok(create_error_response(&format!("Ticket {} not found", ticket_id))),
        }
    }
...
impl ToolHandler for ListTicketsTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        let tickets = Ticket::list_by_project(
            &state.db,
            project_id.as_deref(),
            status.as_deref()
        ).await?;
...
impl ToolHandler for AddTicketCommentTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        info!("Adding comment to ticket {} by worker {}", ticket_id, worker_id);
...
impl ToolHandler for UpdateTicketStageTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        match result {
            Some(_) => Ok(create_success_response(&format!("Updated ticket {} to stage {}", ticket_id, stage))),
            None => Ok(create_error_response(&format!("Ticket {} not found", ticket_id))),
        }
    }
...
impl ToolHandler for CloseTicketTool {
    async fn call(&self, state: &AppState, arguments: Option<Value>) -> crate::error::Result<CallToolResponse> {
        ...
        let ticket_id: String = extract_param(&Some(args.clone()), "ticket_id")?;
        let resolution: String = extract_optional_param(&Some(args.clone()), "resolution")?.unwrap_or_else(|| "completed".to_string());

        info!("Closing ticket {} with resolution: {}", ticket_id, resolution);

        let result = Ticket::close_ticket(&state.db, &ticket_id, "Completed").await?;

        match result {
            Some(_) => Ok(create_success_response(&format!("Closed ticket {} with resolution: {}", ticket_id, resolution))),
            None => Ok(create_error_response(&format!("Ticket {} not found", ticket_id))),
        }
    }
