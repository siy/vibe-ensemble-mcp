...
impl WorkerConsumer {
    ...
    pub async fn start(self) -> Result<()> {
        info!("Starting WorkerConsumer for {}-{}", self.project_id, self.worker_type);
        ...
        loop {
            ...
            if *self.shutdown.read().await {
                info!("WorkerConsumer {}-{} shutting down", self.project_id, self.worker_type);
                break;
            }
    ...
    async fn process_next_ticket(&self) -> Result<bool> {
        ...
        let task_item = match self.state.queue_manager
    ...
    async fn check_database_for_tickets(&self) -> Result<bool> {
        ...
        let tickets = Ticket::get_by_stage_unclaimed(
            &self.state.db,
            &self.project_id,
            &self.worker_type,
        ).await?;
    ...
    async fn process_ticket(&self, ticket_id: &str) -> Result<bool> {
        ...
        info!("Processing ticket {} with worker type {}", ticket_id, self.worker_type);
        ...
        match worker_result {
            ...
            Err(e) => {
                ...
                self.handle_worker_failure(ticket_id, &format!("Worker failed: {}", e)).await?;
                Ok(true)
            }
        }
    }
    ...
    async fn claim_ticket(&self, ticket_id: &str) -> Result<bool> {
        let worker_id = format!("consumer-{}-{}", self.worker_type, Uuid::new_v4().to_string()[..8].to_string());

        // Try to claim the ticket atomically
        let updated = Ticket::claim_for_processing(&self.state.db, ticket_id, &worker_id).await?;

        if updated > 0 {
            debug!("Successfully claimed ticket {} with worker {}", ticket_id, worker_id);
            Ok(true)
        } else {
            Ok(false)
        }
    ...
    async fn spawn_and_wait_for_worker(&self, ticket_id: &str) -> Result<WorkerOutput> {
        let worker_id = format!("worker-{}-{}", self.worker_type, Uuid::new_v4().to_string()[..8].to_string());
    ...
    async fn spawn_worker_simplified(
        ...
        info!("Worker spawning for ticket {} - using placeholder", ticket_id);
        Ok(WorkerOutput {
            outcome: "next_stage".to_string(),
            target_stage: Some("completed".to_string()),
            comment: format!("Processed ticket {} with worker type {}", ticket_id, request.worker_type),
    ...
    async fn process_worker_output(&self, ticket_id: &str, output: &WorkerOutput) -> Result<()> {
        info!("Processing worker output for ticket {}: outcome={}", ticket_id, output.outcome);
        ...
        match output.outcome.as_str() {
            "next_stage" => {
                if let Some(target_stage) = &output.target_stage {
                    ...
                    info!("Ticket {} transitioned to stage {}", ticket_id, target_stage);
                } else {
            ...
            "prev_stage" => {
                ...
                warn!("Worker requested return to previous stage for ticket {}: {}", ticket_id, output.reason);
                // TODO: Implement previous stage logic
            }
            "coordinator_attention" => {
                // Put ticket on hold and create coordinator event
                self.handle_coordinator_attention(ticket_id, &output.reason).await?;
            }
            _ => {
    ...
    async fn enqueue_for_next_stage(&self, ticket_id: &str, target_stage: &str) -> Result<()> {
        ...
        debug!("Added ticket {} to queue for stage {}", ticket_id, target_stage);
        Ok(())
    }
    ...
    async fn handle_worker_failure(&self, ticket_id: &str, error_msg: &str) -> Result<()> {
        ...
        warn!("Ticket {} put on hold due to {}: {}", ticket_id, event_type, error_msg);
        Ok(())
    }
    ...
    async fn handle_coordinator_attention(&self, ticket_id: &str, reason: &str) -> Result<()> {
        ...
        info!("Ticket {} requires coordinator attention: {}", ticket_id, reason);
        Ok(())
    }
}
