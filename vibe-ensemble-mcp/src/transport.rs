...
//!
//! This module provides both stdio and WebSocket transport for MCP protocol communication.
//! Supports single-agent stdio transport and multi-agent WebSocket transport.
        ...
        + Unpin,
{
    ...
    async fn update_initialization_state(
        ...
        // Reuse the initialization logic from StdioTransport
        if !is_outgoing {
            if let Some(init_id) = StdioTransport::is_initialize_request(message)? {
        ...
        + Unpin,
{
    async fn send(&mut self, message: &str) -> Result<()> {
        ...
        StdioTransport::validate_message(message)?;
                    ...
                    Some(Ok(message)) => {
                        match message {
                            Message::Text(text) => {
                                ...
                                if let Err(e) = StdioTransport::validate_message(&text) {
...
/// Transport factory for creating stdio, WebSocket and testing transports
/// Enhanced for multi-transport architecture supporting both stdio and WebSocket
