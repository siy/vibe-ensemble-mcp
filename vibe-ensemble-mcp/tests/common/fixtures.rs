...
impl TestDataFactory {
    ...
    pub fn create_random_agent() -> Agent {
        Agent::builder()
            ...
            .connection_metadata(ConnectionMetadata {
                host: internet::en::IPv4()
                    .fake::<std::net::Ipv4Addr>()
                    .to_string(),
                port: (8000..9000).fake(),
    ...
    pub fn create_random_knowledge(author_id: Uuid) -> Knowledge {
        ...
        Knowledge::builder()
            ...
            .content(
                lorem::en::Paragraphs(2..5)
                    .fake::<Vec<String>>()
                    .join("\n\n"),
            )
            .knowledge_type(types[number::en::NumberWithinRange(0..types.len()).fake()])
            .access_level(
                access_levels[number::en::NumberWithinRange(0..access_levels.len()).fake()],
            )
            .created_by(author_id)
            .build()
        ...
        for _ in 0..message_count {
            ...
            let message = if let Some(recipient_id) = recipient {
                Message::direct(
                    sender,
                    recipient_id,
                    &lorem::en::Sentence(5..15).fake::<String>(),
                )
            } else {
...
mod tests {
    ...
    fn test_development_team_scenario() {
        ...
        assert!(scenario
            .backend_dev
            .capabilities()
            .contains(&"rust_development".to_string()));
        assert!(scenario
            .frontend_dev
            .capabilities()
        ...
        assert!(scenario
            .qa_agent
            .capabilities()
            .contains(&"quality_assurance".to_string()));
    }
    ...
    fn test_issue_backlog() {
        ...
        let priorities: std::collections::HashSet<_> =
            issues.iter().map(|i| i.priority()).collect();
    ...
    fn test_knowledge_repository() {
        ...
        let types: std::collections::HashSet<_> =
            knowledge.iter().map(|k| k.knowledge_type()).collect();
        assert!(types.len() > 1);
    }
