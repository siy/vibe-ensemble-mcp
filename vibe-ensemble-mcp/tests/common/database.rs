...
impl DatabaseTestHelper {
    ...
    pub async fn new() -> Result<Self, sqlx::Error> {
        let temp_file = NamedTempFile::new().map_err(|e| sqlx::Error::Io(e))?;
    ...
    async fn create_test_agents(&self, count: usize) -> Result<Vec<Uuid>, sqlx::Error> {
        ...
        for i in 0..count {
            ...
            sqlx::query!(
                r#"
                INSERT INTO agents (id, name, agent_type, status, capabilities, connection_metadata, created_at, last_seen)
                VALUES (?1, ?2, 'worker', 'active', ?3, ?4, datetime('now'), datetime('now'))
                "#,
                agent_id.to_string(),
                name,
    ...
    async fn create_test_issues(
        ...
        for i in 0..count {
            ...
            sqlx::query!(
                r#"
                INSERT INTO issues (id, title, description, status, priority, assigned_agent_id, created_at, updated_at, tags)
                VALUES (?1, ?2, ?3, 'open', 'medium', ?4, datetime('now'), datetime('now'), '[]')
                "#,
                issue_id.to_string(),
                format!("Test Issue {}", i),
