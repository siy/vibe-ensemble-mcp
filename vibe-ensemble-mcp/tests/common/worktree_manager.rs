...
impl GitWorktreeManager {
    ...
    pub async fn create_worktree(
        ...
        self.active_worktrees
            .insert(unique_name.clone(), worktree_path.clone());
    ...
    async fn list_files_recursive(
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            {
                let subfiles = Box::pin(self.list_files_recursive(&path, base_path)).await?;
                files.extend(subfiles);
            }
        }
        ...
        for entry in status_text.split('\0').filter(|s| !s.is_empty()) {
            if entry.len() >= 3 {
                ...
                match status_code {
                    ...
                    _ => status
                        .other
                        .push((status_code.to_string(), PathBuf::from(file_path))),
                }
            }
        }
...
impl WorktreeTestScenario {
    ...
    pub async fn add_worker_worktree(
        &mut self,
        worker_name: &str,
    ) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let worktree_path = self
            .manager
            .create_worktree(&format!("{}-{}", self.name, worker_name))
            .await?;
...
mod tests {
    ...
    async fn test_file_operations() {
        ...
        assert!(status
            .untracked
            .iter()
            .any(|f| f.file_name().unwrap() == "test.txt"));

        // Commit changes
        manager
            .commit_changes(&worktree_path, "Add test file")
            .await
            .unwrap();
    ...
    async fn test_scenario_workflow() {
        let test_repo = setup_test_repo().await.unwrap();
        let mut scenario = WorktreeTestScenario::new("test-scenario", test_repo.clone())
            .await
            .unwrap();
