...
impl FileSystemVerifier {
    ...
    pub async fn verify_file_exists(
        ...
        if !file_path.is_file() {
            return Err(FileVerificationError::InvalidFileType {
                ...
                actual_type: if file_path.is_dir() { FileType::Directory } else { FileType::Other },
            });
        }
        ...
        if !self.rules.content_matches(&actual_content, expected_content) {
    ...
    pub async fn verify_file_not_exists(&self, file_path: &Path) -> Result<(), FileVerificationError> {
        if file_path.exists() {
            return Err(FileVerificationError::FileNotFound {
                path: file_path.to_path_buf(),
                expected: false,
    ...
    pub async fn verify_directory_structure(
        ...
        for forbidden_file in &expected_structure.forbidden_files {
            ...
            if file_path.exists() {
                return Err(FileVerificationError::ForbiddenFileFound {
                    path: file_path,
                });
            }
        }
        ...
        for (subdir_name, subdir_structure) in &expected_structure.subdirectories {
            ...
            self.verify_directory_structure(&subdir_path, subdir_structure).await?;
        }
    ...
    pub async fn verify_file_creation_time(
        ...
        let created = metadata
            ...
            .map_err(|e| FileVerificationError::ReadError {
                path: file_path.to_path_buf(),
                error: format!("Failed to get creation time: {}", e),
            })?;
    ...
    async fn cache_file_state(&mut self, file_path: &Path) -> Result<(), FileVerificationError> {
        ...
        let content = if metadata.is_file() && metadata.len() <= 1024 * 1024 {
            // Only cache content for files smaller than 1MB
    ...
    async fn perform_single_verification(
        ...
        match verification {
            FileVerification::FileExists { path, expected_content } => {
...
impl VerificationRules {
    ...
    pub fn content_matches(&self, actual: &str, expected: &str) -> bool {
        let actual_normalized = if self.ignore_whitespace {
            actual.chars().filter(|c| !c.is_whitespace()).collect::<String>()
        } else {
            actual.to_string()
        };
        ...
        match self {
            FileVerificationError::FileNotFound { path, expected } => {
                write!(f, "File {}: {}", path.display(), 
                    if *expected { "not found" } else { "unexpectedly exists" })
            }
            FileVerificationError::InvalidFileType { path, expected_type, actual_type } => {
                write!(f, "File {} has wrong type: expected {:?}, got {:?}", 
                    path.display(), expected_type, actual_type)
            }
            ...
            FileVerificationError::SizeMismatch { path, constraint, actual_size } => {
                write!(f, "File {} size {} doesn't meet constraint {:?}", 
                    path.display(), actual_size, constraint)
            }
            FileVerificationError::PermissionMismatch { path, expected, actual } => {
                write!(f, "File {} permissions {:o} don't match expected {:o}", 
                    path.display(), actual, expected)
            }
            FileVerificationError::TimeMismatch { path, .. } => {
                write!(f, "File {} creation time outside expected window", path.display())
            }
...
mod tests {
    ...
    async fn create_test_file(path: &Path, content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn test_file_exists_verification() {
        ...
        let wrong_result = verifier.verify_file_exists(&test_file, "Wrong content").await;
    ...
    async fn test_directory_structure_verification() {
        ...
        create_test_file(&temp_dir.join("file1.txt"), "content1").await.unwrap();
        create_test_file(&temp_dir.join("subdir/file2.txt"), "content2").await.unwrap();
        fs::create_dir_all(&temp_dir.join("empty_dir")).await.unwrap();
        ...
        let mut verifier = FileSystemVerifier::new();
        let result = verifier.verify_directory_structure(&temp_dir, &structure).await;
        
        assert!(result.is_ok());

        // Test with forbidden file
        create_test_file(&temp_dir.join("forbidden.txt"), "forbidden").await.unwrap();
        let forbidden_result = verifier.verify_directory_structure(&temp_dir, &structure).await;
    ...
    async fn test_batch_verification() {
        ...
        create_test_file(&temp_dir.join("file1.txt"), "content1").await.unwrap();
        create_test_file(&temp_dir.join("file2.txt"), "content2").await.unwrap();
    ...
    async fn test_size_constraint() {
        ...
        // Test exact size
        assert!(verifier.verify_file_size(&test_file, SizeConstraint::Exact(5)).await.is_ok());
        assert!(verifier.verify_file_size(&test_file, SizeConstraint::Exact(4)).await.is_err());

        // Test range
        assert!(verifier.verify_file_size(&test_file, SizeConstraint::Range(3, 7)).await.is_ok());
        assert!(verifier.verify_file_size(&test_file, SizeConstraint::Range(6, 10)).await.is_err());

        // Test non-empty
        assert!(verifier.verify_file_size(&test_file, SizeConstraint::NonEmpty).await.is_ok());
