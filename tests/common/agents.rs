...
impl MockAgent {
    ...
    pub async fn broadcast_message(&self, content: &str) {
        let message = Message::broadcast(self.id(), content)
    ...
    pub async fn process_message(&self, message: &Message) -> Option<Message> {
        ...
        if message.content().contains("ping") {
            ...
            Some(Message::direct(
                self.id(),
                message.sender_id(),
                &format!("Agent {} is active with {} capabilities", 
                    self.name(), 
...
impl AgentNetwork {
    ...
    pub async fn deliver_message(&self, sender_id: Uuid, recipient_id: Option<Uuid>, content: &str) {
    ...
    pub async fn simulate_conversation(&self, agent1_id: Uuid, agent2_id: Uuid, message_count: usize) {
        for i in 0..message_count {
            ...
            self.deliver_message(sender_id, Some(recipient_id), &content).await;
        }
    }
...
impl AgentNetworkBuilder {
    ...
    pub fn with_workers(mut self, count: usize, base_name: &str, capabilities: Vec<String>) -> Self {
        for i in 0..count {
...
mod tests {
    ...
    async fn test_agent_network() {
        ...
        network.deliver_message(id1, Some(id2), "Test message").await;
