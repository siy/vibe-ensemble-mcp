...
use vibe_ensemble_mcp::{
    protocol::{JsonRpcRequest, AgentRegisterParams, IssueCreateParams, IssueAssignParams, IssueUpdateParams},
    server::McpServer,
...
pub struct MockCoordinator {
    ...
    transport: Option<Arc<RwLock<dyn Transport>>>,
...
impl MockCoordinator {
    ...
    pub async fn register_with_server(&mut self, server: &McpServer) -> Result<(), Box<dyn std::error::Error>> {
        // Create transport for communication
        let transport = TransportFactory::in_memory();
        ...
        let response = server.handle_message(&serde_json::to_string(&request)?)
            .await?
...
impl MockWorker {
    ...
    pub async fn new(name: &str, capabilities: Vec<String>) -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self {
            id: Uuid::new_v4(),
            name: name.to_string(),
            capabilities,
...
mod tests {
    ...
    async fn test_agent_factory() {
        ...
        assert!(frontend_worker.capabilities.contains(&"frontend".to_string()));
    }
